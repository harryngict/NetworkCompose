...
final class NetworkRouter<SessionType: NetworkSession>: NetworkRouterInterface {
       ...
       loggerInterface: LoggerInterface? = nil)
  {
    ...
    network = NetworkSessionExecutor(
      baseURL: baseURL,
      session: session,
  ...
  var cookieStorage: CookieStorage { network.cookieStorage }
...
private extension NetworkRouter {
  func executeOperation<RequestType>(_ request: RequestType,
                                     ...
                                     operation: @escaping (RequestType,
                                                           [String: String],
                                                           RetryPolicy,
                                                           @escaping (Result<RequestType.SuccessType, NetworkError>) -> Void) -> Void) where RequestType: RequestInterface
  {
